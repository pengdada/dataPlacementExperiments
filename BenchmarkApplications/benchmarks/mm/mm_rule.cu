...
texture<REAL,1,cudaReadModeElementType> tex_A;
texture<REAL,1,cudaReadModeElementType> tex_B;
...
void
computeGold(REAL* C, const REAL* A, const REAL* B, unsigned int hA, unsigned int wA, unsigned int wB)
{
...
__global__ void
mm_kernel( REAL* C, int wA, int wB)
{
  // Block index
  int bx = blockIdx.x;
  int by = blockIdx.y;
  ...
  for (int a = aBegin, b = bBegin;
    ...
    __shared__ REAL As[mm_BLOCK_SIZE][mm_BLOCK_SIZE];

    // Declaration of the shared memory array Bs used to
    // store the sub-matrix of B
    __shared__ REAL Bs[mm_BLOCK_SIZE][mm_BLOCK_SIZE];
...
//  cudaSetDevice(1);
  ...
  unsigned int mem_size_A = sizeof(REAL) * size_A;
  REAL* h_A = (REAL*)malloc(mem_size_A);
  unsigned int size_B = uiWB * uiHB;
  unsigned int mem_size_B = sizeof(REAL) * size_B;
  ...
  unsigned int mem_size_C = sizeof(REAL) * size_C;

  // allocate host memory for the result
  REAL* h_C      = (REAL*) malloc(mem_size_C);
  REAL* h_CUBLAS = (REAL*) malloc(mem_size_C);

  checkCudaErrors(cudaMalloc((void**) &d_A, mem_size_A));
  checkCudaErrors(cudaMalloc((void**) &d_B, mem_size_B));
  
   cudaChannelFormatDesc channelDescA =  cudaCreateChannelDesc<REAL>();
   cudaChannelFormatDesc channelDescB =  cudaCreateChannelDesc<REAL>();
    ...
    // in host memory 
    cudaMemcpyToArray(A_Array, 0, 0, h_A, uiWA * uiHA * sizeof(REAL),
                      cudaMemcpyHostToDevice);
    cudaMemcpyToArray(B_Array, 0, 0, h_B, uiWB * uiHB * sizeof(REAL),
                      cudaMemcpyHostToDevice);
...
/*
  ...
  REAL* reference = (REAL*)malloc(mem_size_C);
