...
// 2-D block mapping: i, j already exist
__global__ void MatMulKernel_naive(Matrix A, Matrix B, Matrix C) {
...
int main(int argc, char* argv[])
{
  ...
  printf("Allocating A, B, C ... size=%d\n",MSIZE );
  A.height = a1;
  A.width = a2;
  ...
  // Invoke kernel
  //-----------------------------
  printf(" Invoking kernel %d times ...block_size=%d\n", ITERATIONS, BLOCK_SIZE);
  // start the event  
  cudaEventRecord(kernel_start, 0);
  // row, col  -> y, x // a bit reversed, ok if consistent
  // assuming divisible by blocksizes
