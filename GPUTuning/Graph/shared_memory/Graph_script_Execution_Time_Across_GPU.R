...
#Make execution time in ms
data$ExecTime = data$ExecTime * 1000
...
plot.memory.impact <- function(data, idx) {
    ...
    #                plot.margin = unit(c(1.5,1,1,0.5), "mm"))#plot.margin = unit(c(-5.5,1,-1,0), "mm"))
    # # p <- p + scale_y_continuous(labels=comma)
    # p <- p + scale_y_continuous(limits=c(0, 7000))
...
filename="Shared_Mem_Execution_Time_Across_GPUs_"
i=1
