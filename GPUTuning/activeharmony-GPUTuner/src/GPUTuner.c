...
/*
 * @GPUTuner, we modify the utility (tuna.c) to add GPU tuning ability
 */
...
/*
 * @GPUTuner, preprocessing functions
 */
int prepare_make_file_argv();
int argv_add_make_file(char* str);
...
/* Containers to keep executable and make file related arguments*/
strlist_t* argv_template = NULL;
strlist_t* argv_template_make_file = NULL;

/*
 * @GPUTuner, data structures to handle and prepare commandline argument
 * for the executable
 */
char* client_bin;
int client_argc = 0;
...
char* make_file_bin = "make";
int make_file_argc = 0;
char** make_file_argv;


char* argv_buf_make_file = NULL;
int argv_buflen_make_file = 0;

int main(int argc, char* argv[])
{
    int i, hresult, line_start, count, j;
    char readbuf[4096];
    char* path;
    char make_file_command[1024];
    FILE* fptr;
    double perf = 0.0;
    ...
    {
        /*Fetch the current selection of tunable parameter*/
        ...
        if (hresult < 0) {
             ...
             * Now from the tuning parameter values, prepare actual parameters
             * for the make file and client executable
             * 
             */
            prepare_make_file_argv(); 
            prepare_client_argv();
        }


        /*
         * @GPUTuner,: This where the app is compiled run
         * We added preprocessing and post processing layers 
         * to set up the compilation and execution process 
         */

        /* 
         * This preprocessing operation compiles the program
         * First it creates a valid make file command based on 
         * tuning parameters
         * Then executes the make command
         */
        if (compile)
        {
            sprintf(make_file_command, "%s ", make_file_bin);
            /* 
             * Add the Environment variable name that is used to 
             * pass the flags to the make file 
             * Flag name is GPUTuner_FLAG
             */
            sprintf(make_file_command, "%s%s", make_file_command, "GPUTuner_FLAG=\"");


            /*for (arglist = argv_template_make_file; arglist != NULL; arglist = arglist->next)
            {
                fprintf(stderr, "argv_template_make_file[]= %s\n", arglist->str);
            }*/

            
            for (j = 0, arglist = argv_template_make_file; (j < make_file_argc) || arglist != NULL; ++j, arglist = arglist->next)
            {
                // fprintf(stderr, "%d\n", j);
                sprintf(make_file_command, "%s%s=%s ", make_file_command, \
                         &arglist->str[1], make_file_argv[j]);
                // fprintf(stderr," value of make_file_argv = %s\n", make_file_argv[j]);
            }
            sprintf(make_file_command, "%s\"\n", make_file_command);
            // fprintf(stderr, "make file command = %s\n", make_file_command);
            /* Do a make clean */
            // fptr = tuna_popen(client_argv[0], "make clean", &pid);
            // popen("make clean", "r");
            /*if (!fptr)
                goto cleanup;
            fclose(fptr);*/
            /* Executing the make file command*/
            // fptr = tuna_popen(client_argv[0], &make_file_command, &pid);
            /*fptr = popen(make_file_command, "r");
            if (!fptr)
                goto cleanup;
            fclose(fptr);*/

            /* @TODO: Change the system() call to popen types of call*/
            system("make clean");
        ...
        if (ah_report(htask, &perf) < 0) {
            fprintf(stderr, "Failed to report performance to server : %s.\n", ah_error());
            goto cleanup;
        }
...
/*
 * @GPUTuner,
 * @Summary, parses the command line argument passed to the GPUTuner
 * After parsing sets up necessary parameter (e.g. client_argc) and
 * allocates necessary memory to keep different types of parameters
 * (e.g. client_argv, make_file_argv) 
 */
void parseArgs(int argc, char* argv[])
{
    int i, stop = 0, chapel = 0,  no_make_file_param = 0; //compile = 0,
    char* arg;
    bundle_info_t* bun;
    ...
    /*
     * Iterate through the argv[] list passed to the GPUTuner, parse them
     * and set up appropiate varibles
     */
    for (i = 1; i < argc && *argv[i] == '-' && !stop; ++i) 
    {
        ...
        while (*arg != '\0') 
        {
            switch (*arg) 
            {
                ...
                /*Handling different types of tuning parameters*/
                ...
                /*
                 * Getting the max number of iteration from command line
                 * and setting up the max_loop parameter
                 */
                case 'n': ++arg;
                    errno = 0;
                    if (*arg == '=' || isdigit(*arg)) 
                ...
                case '-':
                    ...
                    /* Flag to signal that we want the app to be compiled */
    ...
    /*
     * @GPUTUner, @TODO
     * In here we would like to handle makefile and makefile related params
     * Just follow the same approach that is used to set up client executable
     */

    /* 
     * We want to compile the code, make necessary preparation
     * In other words parse tunable parameters that'll be used for compilation
     * These params start with ~
     */

    /* @TODO: There was problem in this loop*/
    if (compile)
    {
        while (i < argc) 
        {
            /* 
             * First check if the param is for make file or not
             * If yes, try to add it to the argv_template_make_file
             * If not, then that means all the params related to make file
             * is done processing, so get out of the while loop,
             * and move on to pasrse executable/client and it's params
             */
            if (*argv[i] == '~')
            {
                if (argv_add_make_file(argv[i]) < 0)
                exit(-1);
            }
            else
            {
            ...
            for (arg = argv[i]; *arg != '\0'; ++arg) 
            {
                ...
                /* 
                 * If the argument doesn't start with '~' that means 
                 * we don't have make file parameters anymore
                 */
                else
                {
                    no_make_file_param = 1;
                    break;
                }
            }
    ...
    /*
     * @GPUTUner, get the executable name and set up
     * The executbale name looks like this execName_parentPID
     */
    ...
    /* 
     * Parse tunable parameters that'll be used as command line option
     * for the executable
     * These params start with %
     */
    while (i < argc) 
    ...
    /* 
     * @GPUTuner, now append all the tunable parameters that
     * have not been used in make file or executable argument
     * to the executbale argument
     *
     * WARNING: Ideally we do not want this
     */
    for (i = 0; i < bcount; ++i) {
        if (binfo[i].used == 0) {
            if (verbose)
    ...
    /* Allocate memory for executable argv*/
    ...
    /* @GPUTuner, allocate memory for make file argv*/
    make_file_argv = malloc(sizeof(char*) * (make_file_argc + 1));
    if (make_file_argv == NULL) {
        perror("Malloc error");
        exit(-1);
    }
}
...
/*
 * @GPUTuner,
 * @Summary, From the tuning parameter values, 
 * prepares valid commandline arguments for the client/executable,
 * We do this because not necessarily the tuning parameter value 
 * can be readily used, hence this preprocessing
 *
 * This is kind of a preprocessing layer
 * One may need to modify this function for different types of tuning
 */
int prepare_client_argv()
{
    unsigned int i = 0;
 ...
 * @GPUTuner,
 * @Summary, From the tuning parameter values, 
 * prepares valid commandline arguments for the make file,
 * We do this because not necessarily the tuning parameter value 
 * can be readily used, hence this preprocessing
 *
 * This is kind of a preprocessing layer
...
int prepare_make_file_argv()
{
    ...
    for (arglist = argv_template_make_file; arglist != NULL; arglist = arglist->next) {
        make_file_argv[i++] = argv_buf_make_file + count;
        ...
        for (arg = arglist->str; *arg != '\0'; ++arg) {
            if (*arg == '~') {
                ...
                remainder = 0;
                if (count < argv_buflen_make_file)
                    remainder = argv_buflen_make_file - count;

                switch (bun->type) {
                case HVAL_INT:  len = snprintf(argv_buf_make_file + count, remainder,
                                               "%ld", *(long*)bun->data);
                    break;
                case HVAL_REAL: len = snprintf(argv_buf_make_file + count, remainder,
                                               "%lf", *(double*)bun->data);
                    break;
                case HVAL_STR:  len = snprintf(argv_buf_make_file + count, remainder,
                                               "%s", *(char**)bun->data);
                    break;
                default:        assert(0 && "Invalid parameter type.");
                }
                ...
                if (*arg == '\\')
                    ++arg;
                if (count < argv_buflen_make_file)
                    *(argv_buf_make_file + count) = *arg;
                ++count;
            }
        }
        if (count < argv_buflen_make_file)
            *(argv_buf_make_file + count) = '\0';
        ++count;
    }
    make_file_argv[i] = NULL;

    if (count > argv_buflen_make_file) {
        argv_buf_make_file = realloc(argv_buf_make_file, count);
        if (argv_buf_make_file == NULL) {
            perror("Realloc error");
            return -1;
        }
        argv_buflen_make_file = count;
        return prepare_make_file_argv();
    }
    return 0;
}
...
/*
 * @GPUTuner
 * @Summary, add the parametes that are used as executable argument
 * in a linked list called "argv_template"
 * We can use this linked list to keep track of the params
 * that will be used for executable argument
 */
...
/*
 * @GPUTuner
 * @Summary, add the parametes that are used as make file argument
 * in a linked list called "argv_template_make_file"
 * We can use this linked list to keep track of the params
 * that will be used for make file argument
 */
int argv_add_make_file(char* str)
{
    static strlist_t** tail;

    if (argv_template_make_file == NULL)
        tail = &argv_template_make_file;
...
/*
 * @GPUTuner,
 * If a tunable varible (e.g. memtype) is passed to the executbale
 * as a commandline arg, then we write %memtype after the exectutable
 *
 * @Summary, parse that tunable param name and get necessary info
 * on that variable (e.g. posible values)
 */
...
bundle_info_t* tuna_bundle_get(char** name)
{
    ...
    /* Search for executable's arguments */
    if (**name == '%') 
    {
        ++(*name);
        if (**name == '{') {
    ...
    /* @GPUTUner, Added support for make file's arguments*/
    if (**name == '~') 
    {
        ++(*name);
        if (**name == '{') {
    ...
    /* 
     * @GPUTUner, Loop through the binfo array [array of tunable parameters]
     * compare the name, if the requested parameter is in the bundle [binfo]
     * return the requested parameter's binfo struct 
     */
    for (i = 0; i < bcount; ++i) {
